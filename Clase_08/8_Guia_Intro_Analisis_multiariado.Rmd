...
**{gridExtra}**
Librería que implementa métodos para elaborar panel con multiples gráficas.
...
Habilite los paquetes **readxl**, **dplyr**, **vegan**, **psych** y  **gridExtra** usando la función **library()**.
...
**Ejercicio 1.** Importar y explorar datos cuantitativos continuos
...
a) Para las tres variables cuantitativas continuas Depth, Pollution y Temperature elabore 3 variables derivadas Depth1, Pollution1 y Temperature1 con su *valor estandarizado* como la diferencia de cada valor por la media **mean()** y dividiendo por la desviasión estandar **sd()**. Use la función **mutate()** de la librería **dplyr**

```{r}
# Crea nuevas variables
val_estandarizado <- bioenv %>%
 select(Sitio, Depth, Pollution, Temp.) %>%
 mutate(
Depth1 = (Depth - mean(Depth)) / sd(Depth),
Pollution1 = (Pollution - mean(Pollution)) / sd(Pollution),
Temperature1 = (Temp. - mean(Temp.)) / sd(Temp.))
```
...
c) Calcule la matriz de distancia euclideana estandarizada a partir de los datos Depth1, Pollution1 y Temperature1. Use la función **dist()**.
...
**Ejercicio 3.** Explorar datos cuantitativos discretos.

a) Realice un gráfico de barras para cada una de las variables discretas de abundancia. Use ggplot() y geom_bar(). Una todas las gráficas con el comando grid.arrange() del paquete gridExtra. Use el siguiente tutorial como referencia http://www.sthda.com/english/wiki/wiki.php?id_contents=7930
...
**Ejercicio 4.** Matriz de similaridad Bray-Curtis
...
Exprese el índice como porcentaje y compare con la correlación estimada anteriormente en el ejercicio a.
