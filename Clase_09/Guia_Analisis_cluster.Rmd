...
Elabore un archivo Rmarkdown o file con extensión **.R** en Rstudio o Rstudio.cloud.

Habilite los paquetes **readxl**, **dplyr**, **vegan**, **psych**, **cluster** y **factoextra** usando la función **library()**.
...
Para cada estación los investigadores calcularon los siguientes índices ecológicos: 
...
a) Importe la hoja 1 set de datos **poliquetos.xlsx** usando la función *read_excel()* del paquete *readxl*. No olvide usar el argumento *Sheet=1*. Explore el set de datos usando las funciones *summary()*. Compruebe que todas las variables numéricas están expresadas como número y que las variable Estaciones sea un factor. Caso contrario realice los cambios correspondientes con las funciones **as.numeric()** y **as.factor()**.
...
a) Para las tres variables cuantitativas continuas S, H y J elabore 3 variables derivadas S1, H1 y J1 con su *valor estandarizado* como la diferencia de cada valor por la media **mean()** y dividiendo por la desviación estandar **sd()**. Use la función **mutate()** de la librería **dplyr**
...
Extrae primeros 6 elementos de la matriz
```{r}
...
d) Note que el análisis de cluster por k-means no nos dice nada acerca de cual es el número óptimo de cluster y todos parecen dar una buena representación de los datos. 
